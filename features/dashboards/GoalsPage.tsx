        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    ...
                    <div className="w-16 h-16 rounded-full border-4 border-blue-100 border-t-blue-600 flex items-center justify-center">
                        <TrendingUp size={24} className="text-blue-600" />
                    </div>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-sm text-gray-500 font-medium mb-1">On Track</p>
                        <h3 className="text-3xl font-bold text-gray-900">12 <span className="text-sm text-gray-400 font-normal">/ 18</span></h3>
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-sm text-gray-500 font-medium mb-1">At Risk</p>
                        <h3 className="text-3xl font-bold text-gray-900">3 <span className="text-sm text-gray-400 font-normal">Objectives</span></h3>
                    </div>
                    ...
                    <div key={obj.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        >
                            <div className="flex items-center space-x-4 flex-1">
                                <div className={`p-2 rounded-lg transition-transform duration-200 ${obj.expanded ? 'rotate-90' : ''}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center mt-1 space-x-3">
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${getStatusColor(obj.status)} uppercase tracking-wide`}>
                                            {obj.status.replace('-', ' ')}
                                        </span>
                                        <span className="text-xs text-gray-400">Due Dec 31, 2024</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {/* Key Results */}
                        {obj.expanded && (
                            <div className="bg-gray-50/50 border-t border-gray-100 p-6 pl-24 space-y-3">
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Key Results</h4>
                                {obj.keyResults.map(kr => (
                                    <div key={kr.id} className="bg-white p-4 rounded-xl border border-gray-200 flex items-center justify-between group hover:shadow-sm transition-shadow">
                                        <div className="flex items-center space-x-4">
                                            <div className={`w-2 h-2 rounded-full ${getConfidenceColor(kr.confidence)}`} title={`Confidence: ${kr.confidence}`}></div>
                                            <span className="font-medium text-gray-800">{kr.title}</span>
                                            <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded">{kr.owner}</span>
                                        </div>
                                ...
                                <button className="mt-2 text-sm text-blue-600 font-medium hover:text-blue-700 flex items-center px-2 py-1 rounded hover:bg-blue-50 transition-colors">
                                    <Plus size={14} className="mr-1" /> Add Key Result
                                </button>
                            </div>
                        )}
                    </div>
