        ...
        <div className={`h-full w-[340px] shadow-2xl border-l flex flex-col animate-in slide-in-from-right duration-300 relative ${darkMode ? 'bg-[#1a1d24] border-gray-700' : 'bg-white border-gray-200'}`} ref={menuRef}>
            ...
            <div className="px-5 py-3">
                <div className="relative group">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 group-focus-within:text-gray-600 dark:group-focus-within:text-gray-300 transition-colors" size={15} />
                    <input
                        type="text"
                        placeholder="Search for new or existing fields"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className={`w-full pl-9 pr-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-500/10 transition-all ${darkMode ? 'bg-gray-800 border-gray-700 text-gray-200 placeholder-gray-500 focus:border-gray-500' : 'bg-white border-gray-200 focus:border-stone-400 placeholder-gray-400'}`}
                        autoFocus
                    />
                </div>
            </div>
