...
export const useRoomBoardData = (storageKey: string) => {
    ...
    // Reload data when storageKey changes or storage events occur
    useEffect(() => {
        const loadData = () => {
            try {
                if (typeof window !== 'undefined') {
        ...
        const handleStorage = (e: StorageEvent) => {
            if (e.key === storageKey) loadData();
        };

        const handleCustomStorage = (e: CustomEvent) => {
            if (e.detail?.key === storageKey) loadData();
        };

        window.addEventListener('storage', handleStorage);
        window.addEventListener('local-storage-update', handleCustomStorage as EventListener);

        return () => {
            window.removeEventListener('storage', handleStorage);
            window.removeEventListener('local-storage-update', handleCustomStorage as EventListener);
        };
    }, [storageKey]);
