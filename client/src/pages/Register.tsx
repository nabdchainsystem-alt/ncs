...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (name.trim() && name.trim().length < 2) next.name = 'Use at least 2 characters';
    if (!email.trim()) next.email = 'Email is required';
    if (!password) next.password = 'Password is required';
    if (password && password.length < 8) next.password = 'Password must be at least 8 characters';
    if (!confirmPassword) next.confirmPassword = 'Confirm your password';
    if (password && confirmPassword && password !== confirmPassword) next.confirmPassword = 'Passwords must match';
    setErrors(next);
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="w-full shadow-xl border-gray-200/70 dark:border-gray-800">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Create your account</CardTitle>
            <CardDescription className="text-sm text-gray-500 dark:text-gray-400">
              Join the procurement workspace in a few seconds.
            </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit} className="space-y-5">
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-gray-400">
              Passwords must include a letter and number. We store credentials with strong hashing and a short-lived session cookie.
            </p>
          </CardContent>
