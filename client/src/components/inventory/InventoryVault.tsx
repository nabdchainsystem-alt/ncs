...
const folders = ['Photos','Safety Sheets','Certificates','Vendor Catalog'] as const;
...
export default function InventoryVault() {
  ...
  return (
    <div className="arch-card p-4">
      <div className="text-sm font-bold mb-3">Inventory Attachments Vault</div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        {items.slice(0,4).map((it)=> (
          <button key={it.id} className="arch-card p-4 text-center hover:shadow-lg-soft" onClick={()=> setOpen(it)}>
          ...
          <div className="w-full max-w-4xl rounded-2xl bg-white shadow-2xl border animate-slide-up" onClick={(e)=> e.stopPropagation()}>
            ...
            <div className="p-3 overflow-auto">
              <table className="u-table text-sm">
                ...
                <tbody>
                  {buildRows(open.code).map((r,i)=> (<tr key={i}><td>{r.name}</td><td>{r.type}</td><td>{r.updated}</td><td>{r.size}</td></tr>))}
                </tbody>
              </table>
            </div>
          </div>
